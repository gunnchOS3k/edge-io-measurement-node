package org.gunnchos.edgeio

data class ConsentState(
    val summaryAcknowledged: Boolean = false,
    val status: String = "pending", // pending|active|withdrawn
    val receiptId: String? = null,
    val capturedAtIso: String? = null,
)

data class MetricSample(
    val timestampIso: String,
    val latencyMs: Double?,
    val jitterMs: Double?,
    val packetLossPct: Double?,
    val uploadMbps: Double?,
    val downloadMbps: Double?,
    val networkType: String,
    val cpuPct: Double?,
    val memoryPct: Double?,
    val batteryPct: Double?,
    val charging: Boolean?,
    val thermalState: String,
    val localEdgeResponseMs: Double?,
    val signalDbm: Double?,
    val qualityFlags: List<String>,
    val unavailable: Map<String, String> = emptyMap(),
)

data class SessionState(
    val runId: String,
    val siteId: String,
    val profile: String,
    val startedAtEpochMs: Long? = null,
    val endedAtEpochMs: Long? = null,
    val plannedDurationSeconds: Double = 60.0,
    val samples: List<MetricSample> = emptyList(),
    val sessionMode: SessionMode = SessionMode.CALIBRATION,
    val calibrationOnly: Boolean = false,
    val rehearsalOnly: Boolean = false,
    val collectionDayId: String = "calibration_day",
    val namedTestZone: String = "zone_calibration",
    val locationCategory: String = "home_or_private_indoor",
    val indoorOutdoor: String = "indoor",
    val stationaryOrMoving: String = "stationary",
    val networkCondition: String = "wifi_normal",
    val detectedNetworkTransport: String = "unavailable",
    val declaredNetworkCondition: String = "wifi_normal",
    val assignmentId: String? = null,
    val assignmentHash: String? = null,
    val matrixCellId: String? = null,
    val protocolVersion: String? = null,
    val environmentalNotes: String = "",
    val transportCompatible: Boolean = true,
    val protocolDeviation: String? = null,
    /** Consent frozen at session start so later withdrawal cannot rewrite collection evidence. */
    val consentStatusAtStart: String? = null,
    val consentReceiptIdAtStart: String? = null,
    val consentCapturedAtIsoAtStart: String? = null,
)
